...
| `bootc switch` fails: "image not found" | Requested branch tag has not been published to GHCR         | Build `main` for `:stable-testing`; approve promotion to `stable` for `:stable`; verify the image under Packages                 |
