...
@test "_build-bib: escalates only for the container run and the artifact move" {
    ...
    grep -qE '^chown -R [0-9]+:[0-9]+ output/' "${SUDO_LOG}"
}

@test "_build-bib: succeeds when USER is undefined and chowns to the running uid:gid" {
    # Regression for #374: the recipe used $USER, which is unset in
    # container/cron/systemd/env -i shells, so under `set -u` it aborted here
    # AFTER the build finished and left output/ owned by root. It now resolves
    # ownership from the kernel with `id -u`/`id -g`.
